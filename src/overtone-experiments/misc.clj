...
(defn read-score
  ...
  (let [time-sig (first score)
        notes    (filterv #(not (contains? #{1 3/4} %)) score)
        inst     p/piano]
    (reset! cur-time nil)
    (reduce
     (fn [acc n]
  ...
  (reduce (fn [acc n]
            (if (and (<= 3 (count (name n)))
               ...
               (let [note     (make-note n [])
                     ...
                     new-note (str (name (:name note)) txp (:rhythm note))]
                 (conj acc new-note))
               (conj acc n)))
          (conj [] (first score))
          (drop 1 score)))
...
(comment
  ...
  (def minuet-in-c-melody
    '[4/4
      R04 C42 R08 B316 C416              |
      D48 C48 Bb38 Ab38 F#38 G38 Ab38    |
      R04 G48 F48 Eb48 D48 C48 B38 Ab38  |
      C58 Bb48 Ab48 G48 F48 Eb48 D48 C48 ||])
